...
class TaskList extends LitElement {
  ...
  private getCategoryColor(id: number) {
    return this.categories.find((c) => c.id === id)?.color || 'gray'
  }

  private handleDelete(id: number) {
    this.dispatchEvent(new CustomEvent('delete-task', { detail: id, bubbles: true, composed: true }))
  }

  render() {
    if (this.tasks.length === 0) {
      return html`<p class="text-center text-gray-500 py-8">No tasks found.</p>`
    }
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                ...
                <div class="flex items-center gap-3 text-xs text-gray-400">
                  <span style="color: ${this.getCategoryColor(task.categoryId)}">
              ...
              <div class="flex gap-2 ml-4">
                <a
                  ...
                  class="px-3 py-1.5 text-xs bg-blue-50 text-blue-600 rounded hover:bg-blue-100"
                >
                  Edit
                </a>
                <button
                  @click=${() => this.handleDelete(task.id)}
