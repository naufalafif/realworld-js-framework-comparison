...
export default function TaskList(props: Props) {
  const getCategoryName = (id: number) => props.categories.find((c) => c.id === id)?.name || 'Unknown'
  const getCategoryColor = (id: number) => props.categories.find((c) => c.id === id)?.color || '#gray'

  return (
    <Show when={props.tasks.length > 0} fallback={<p class="text-center text-gray-500 py-8">No tasks found.</p>}>
      <div class="space-y-3">
